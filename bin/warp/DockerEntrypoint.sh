...
healthcheck_loop() {
  ...
  local proxy; proxy="$(proxy_addr_from_bind)"
  ...
  while :; do
    if ! output=$(curl --socks5-hostname "$proxy" -sf --max-time "$timeout" "$url"); then
      log ERROR "Healthcheck: Failed to reach $url via SOCKS5 ($proxy). Triggering container stop."
      kill 1
    fi
    # Поддержим возможное поле "fails" в JSON-ответе (если вдруг используется ваш тестовый URL)
    fails=$(printf '%s\n' "$output" | grep -o '"fails":[0-9]*' | cut -d: -f2 | tr -d '\r')
    if [ -n "$fails" ]; then
      for count in $fails; do
        if [ "$count" -ge "$max_fails" ]; then
          log ERROR "Healthcheck: fails=$count ≥ $max_fails — restarting."
          kill 1
        fi
      done
    else
      log DEBUG "Healthcheck: Healthy (no 'fails' field)."
    fi
    sleep "$interval"
  done
}
